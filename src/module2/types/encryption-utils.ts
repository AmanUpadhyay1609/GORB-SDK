 ...
 * Ported from sdk2/src/crypto/utils.ts
 */
...
// Import base58 utilities from existing SDK
import { bytesToBase58, base58ToBytes } from "../../utils/base58";
 ...
 */
export function ed25519ToCurve25519PublicKey(
...
export function performKeyExchange(
  ...
  for (let i = 0; i < 32; i++) {
    const a = privSeed[i] ?? 0;
    const b = publicKey[i] ?? 0;
    combined[i] = a ^ b;
  }
...
/**
 * Derive a symmetric shared secret from two public keys (demo fallback).
 * This produces the same 32-byte secret for (pubA, pubB) and (pubB, pubA).
 */
export function deriveSymmetricSecret(
  ...
  const concat = Buffer.concat(a.compare(b) <= 0 ? [a, b] : [b, a]);
  const hash = createHash("sha256").update(concat).digest();
...
export function verifySignature(
  ...
  const signatureBytes =
    typeof signature === "string" ? base58ToBytes(signature) : signature;
  const publicKeyBytes =
    typeof publicKey === "string" ? base58ToBytes(publicKey) : publicKey;
...
export function generateId(...inputs: (string | Uint8Array)[]): string {
  ...
  return bytesToBase58(hash.digest());
}
